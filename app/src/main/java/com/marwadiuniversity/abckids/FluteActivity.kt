...
class FluteActivity : AppCompatActivity() {
    ...
    private fun createEnhancedFluteComponents(container: RelativeLayout) {
        try {
            ...
            // Add bamboo decorations first, then place holes above everything.
            createEnhancedDecorations(container)

            // Create enhanced finger holes with notes (top-most interactive layer)
        ...
        notes.forEachIndexed { index, note ->
            try {
                val hole = View(this).apply {
                    ...
                    translationZ = dpToPx(12).toFloat()
                    isClickable = true
                    isFocusable = true
                    setOnClickListener {
                        playNote(note)
                ...
                val label = TextView(this).apply {
                    ...
                    translationZ = dpToPx(14).toFloat()
                }
    ...
    private fun animateEnhancedHolePress(button: View) {
        try {
            button.animate()
                .scaleX(0.85f)
                .scaleY(0.85f)
